...
async function handleMention(text: string, channel: string) {
  ...
  try {
    ...
    await postSlackMessage(channel, '❌ Couldn\'t extract a URL. Try: @Parallax check https://example.com');
    return;
  }
  ...
  // Fetch page HTML (no browser needed)
  let pageContent = '';
  try {
  ...
  for (const persona of SLACK_PERSONAS) {
    try {
      const { object } = await generateObject({
        ...
        prompt: `${persona.systemPrompt}\n\nYou are visiting: ${url}\nComplaint: "${parsed.intent}"\n\nPage content:\n${pageContent}\n\nAnalyze from your persona's perspective. Score 1-10, provide summary, 3 pain points, 2 highlights. Be specific.`,
      });
    ...
    try {
      ...
      if (ghMatch) {
        ...
        const ghRes = await fetch(`https://api.github.com/repos/${owner}/${cleanRepo}/issues`, {
          ...
          body: JSON.stringify({ title: issueTitle, body: issueBody, labels: ['parallax', 'ux-bug'] }),
        });
...
export async function POST(req: NextRequest) {
  ...
  if (body.event?.type === 'app_mention') {
    ...
    // Do all work before responding — Slack retries are handled above
